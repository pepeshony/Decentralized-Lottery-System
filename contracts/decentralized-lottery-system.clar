...
(define-read-only (get-ticket-owner (id uint) (ticket-number uint))
  (map-get? lottery-tickets { lottery-id: id, ticket-number: ticket-number })
)

(define-read-only (get-player-ticket-count (id uint) (player principal))
  (default-to 
    { ticket-count: u0 }
    (map-get? player-tickets { lottery-id: id, player: player })
  )
)

(define-read-only (get-player-participation (id uint) (player principal))
  (map-get? lottery-participants { lottery-id: id, player: player })
)
...
(define-public (buy-ticket (id uint))
  (let 
    ...
    (begin
      ...
      (asserts! (< current-ticket-count (var-get max-tickets-per-player)) ERR_TOO_MANY_TICKETS)
      (asserts! (>= (stx-get-balance tx-sender) ticket-cost) ERR_INSUFFICIENT_PAYMENT)
      
      (try! (stx-transfer? ticket-cost tx-sender (as-contract tx-sender)))
      
      (map-set lottery-tickets 
        { lottery-id: id, ticket-number: new-ticket-number }
        { owner: tx-sender }
      )
      
      (map-set player-tickets
        { lottery-id: id, player: tx-sender }
        { ticket-count: (+ current-ticket-count u1) }
      )
      
      (match (map-get? lottery-participants { lottery-id: id, player: tx-sender })
...
(define-public (draw-winner (id uint))
  (let 
    (
      ...
      (winner-info (unwrap! (map-get? lottery-tickets { lottery-id: id, ticket-number: winning-ticket }) ERR_NO_TICKETS_SOLD))
    ...
    (begin
      ...
      (ok { winner: winner, prize: winner-prize, winning-ticket: winning-ticket })
    )
  )
)
...
(define-read-only (predict-winner (id uint))
  (match (map-get? lotteries id)
    lottery
      (if (and (is-lottery-ended id) (not (get is-drawn lottery)))
        (let 
          (
            (total-tickets (get total-tickets lottery))
            (end-block (get end-block lottery))
          ...
          (if (> total-tickets u0)
            (ok { winning-ticket: winning-ticket, can-draw: true })
            ERR_NO_TICKETS_SOLD
          )
        )
        (ok { winning-ticket: u0, can-draw: false })
      )
    ERR_LOTTERY_NOT_ACTIVE
  )
)

(define-read-only (can-buy-more-tickets (id uint) (player principal))
  (let 
    (
...
(define-read-only (check-ticket-ownership (id uint) (ticket-number uint) (player principal))
  (match (map-get? lottery-tickets { lottery-id: id, ticket-number: ticket-number })
    ticket-info (is-eq (get owner ticket-info) player)
    false
  )
)

(define-read-only (get-player-total-spent (id uint) (player principal))
  (match (map-get? lottery-participants { lottery-id: id, player: player })
    participation (get total-spent participation)
    u0
  )
)

(define-read-only (calculate-win-probability (id uint) (player principal))
  (match (map-get? lotteries id)
    lottery
      (let 
        (
...
(define-read-only (validate-lottery-params (duration uint) (price uint))
  (ok {
    duration-valid: (and (> duration u10) (<= duration u1000)),
    price-valid: (> price u0),
    all-valid: (and (> duration u10) (<= duration u1000) (> price u0))
  })
)

(define-public (create-custom-lottery (duration-blocks uint) (custom-ticket-price uint))
  (let 
    (
      (current-id (+ (var-get lottery-counter) u1))
      (start-block stacks-block-height)
  ...
  (match (map-get? lotteries id)
    lottery
      (ok {
        ...
        is-drawn: (get is-drawn lottery),
        can-draw: (and (is-lottery-ended id) (not (get is-drawn lottery)) (> (get total-tickets lottery) u0))
      })
    ERR_LOTTERY_NOT_ACTIVE
  )
)
...
(define-read-only (get-total-prize-info (id uint))
  (match (map-get? lotteries id)
    lottery
      (let ((total-prize (get total-prize lottery)))
        (ok {
...
(define-read-only (get-ticket-range-info (id uint) (start-ticket uint) (end-ticket uint))
  (ok {
    lottery-id: id,
    start-ticket: start-ticket,
    end-ticket: end-ticket,
    range-size: (if (>= end-ticket start-ticket) (+ (- end-ticket start-ticket) u1) u0)
  })
)

(define-public (refund-emergency (id uint) (player principal))
  (let 
    (
      (lottery (unwrap! (map-get? lotteries id) ERR_LOTTERY_NOT_ACTIVE))
      (participation (unwrap! (map-get? lottery-participants { lottery-id: id, player: player }) ERR_NOT_DRAWN))
    ...
    (begin
      ...
      (map-delete lottery-participants { lottery-id: id, player: player })
      (map-delete player-tickets { lottery-id: id, player: player })
...
(define-read-only (calculate-ticket-cost (id uint) (quantity uint))
  (match (map-get? lotteries id)
    lottery
      (ok (* (get ticket-price lottery) quantity))
    ERR_LOTTERY_NOT_ACTIVE
  )
)
...
(define-public (buy-tickets-batch (id uint) (tickets-to-buy uint))
  (let 
    (
      (lottery (unwrap! (map-get? lotteries id) ERR_LOTTERY_NOT_ACTIVE))
    ...
    (begin
      (asserts! (is-lottery-active id) ERR_LOTTERY_NOT_ACTIVE)
      (asserts! (<= (+ current-count tickets-to-buy) (var-get max-tickets-per-player)) ERR_TOO_MANY_TICKETS)
